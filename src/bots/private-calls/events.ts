...
import { Interaction, VoiceState, VoiceChannel, Client, Events } from 'discord.js';
import { database } from '../../shared/services/database';
import { logger } from '../../shared/services/logger.service';
import { PRIVATE_CALLS_SERVICE } from './private-calls.service';
import { SlashCommandBuilder } from '@discordjs/builders';
import { PermissionsBitField } from 'discord.js';

export function setupPrivateCallsEvents(client: Client) {
    logger.info('📞 Módulo Private Calls: Eventos inicializados');

    client.on(Events.InteractionCreate, async (interaction: Interaction) => {
        await PRIVATE_CALLS_EVENTS.onInteractionCreate(interaction);
    });

    client.on(Events.VoiceStateUpdate, async (oldState: VoiceState, newState: VoiceState) => {
        await PRIVATE_CALLS_EVENTS.onVoiceStateUpdate(oldState, newState);
    });
}

export const PRIVATE_CALLS_EVENTS = {
    name: 'PrivateCalls',
    ...
    async onInteractionCreate(interaction: Interaction) {
        ...
        const member = interaction.member as any; // Cast needed for roles
        ...
        if (config.private_calls_allowed_roles && config.private_calls_allowed_roles.length > 0) {
            const hasRole = member.roles.cache.some((r: any) => config.private_calls_allowed_roles!.includes(r.id));
            const isManager = config.private_calls_manager_role && member.roles.cache.has(config.private_calls_manager_role);

            if (!hasRole && !isManager && !member.permissions.has(PermissionsBitField.Flags.Administrator)) {
        ...
        if (subcommand === 'criar') {
            ...
            const channel = await PRIVATE_CALLS_SERVICE.createPrivateChannel(
                interaction.guild!,
                member,
                config.private_calls_category_id
            );
        ...
        let call = await database.getPrivateCallByOwner(guildId, member.id);
        ...
        const channel = interaction.guild!.channels.cache.get(call.channel_id) as any;
        if (!channel) {
            await database.deletePrivateCall(call.channel_id);
        ...
        switch (subcommand) {
            ...
            case 'bloquear':
                ...
                await database.updatePrivateCall(channel.id, { is_open: false });
                break;
            ...
            case 'desbloquear':
                ...
                await database.updatePrivateCall(channel.id, { is_open: true });
                break;
            ...
            case 'limite':
                ...
                await database.updatePrivateCall(channel.id, { member_limit: limit });
                break;
        }
    },

    async onVoiceStateUpdate(oldState: VoiceState, _newState: VoiceState) {
        // Handle Auto-Cleanup
        ...
        if (channelLeft) {
            ...
            const isPrivateCall = await database.getPrivateCall(channelLeft);
            if (isPrivateCall) {
                const channel = guild.channels.cache.get(channelLeft) as VoiceChannel;
                // Force cast members to any to avoid TS issue with ThreadMemberManager union
                if (channel && (channel.members as any).size === 0) {
                    await PRIVATE_CALLS_SERVICE.deletePrivateChannel(channelLeft, guild);
                }
            }
        }
    }
