...
const server_builder_service_1 = require("../../shared/services/server-builder.service");
const branding_service_1 = require("../../shared/services/branding.service");
const analytics_service_1 = require("../../shared/services/analytics.service");
const partnerships_service_1 = require("../../shared/services/partnerships.service");
const events_1 = require("../welcome/events");
const events_2 = require("../automod/events");
const events_3 = require("../tickets/events");
const engine_1 = require("../custom-commands/engine");
const service_1 = require("../custom-commands/service");
const events_4 = require("../private-calls/events");
let dbConnected = false;
// ═══════════════════════════════════════════════════════════════════════════
// 🔐 VALIDAÇÃO
// ═══════════════════════════════════════════════════════════════════════════
services_1.config.validate(['DISCORD_TOKEN_AGENTE_IA', 'OPENAI_API_KEY']);
const TOKEN = services_1.config.get('DISCORD_TOKEN_AGENTE_IA');
const OPENAI_API_KEY = services_1.config.get('OPENAI_API_KEY');
const LLM_BASE_URL = services_1.config.getOptional('LLM_BASE_URL') || 'https://api.openai.com/v1';
const MODELO_IA = services_1.config.getOptional('MODELO_IA') || 'gpt-3.5-turbo';
const CATEGORIA_ASSISTENTE = services_1.config.getOptional('CATEGORIA_ASSISTENTE');
const CANAL_CHAT_GERAL = services_1.config.getOptional('CANAL_CHAT_GERAL');
// const OWNER_ROLE_ID = config.get('OWNER_ROLE_ID'); // Removed for multi-tenancy
const STAFF_ROLE_ID = services_1.config.getOptional('STAFF_ROLE_ID');
// ═══════════════════════════════════════════════════════════════════════════
// 🎨 CONSTANTES
// ═══════════════════════════════════════════════════════════════════════════
...
class LLMService {
    ...
    constructor() {
        if (OPENAI_API_KEY) {
            ...
            this.mode = 'API';
            services_1.logger.info(`🧠 LLM Service iniciado em modo API (${LLM_BASE_URL})`);
        }
        else {
            services_1.logger.warn('⚠️ SEM CHAVE DE API. LLM Service em modo MOCK.');
        }
    }
    async gerarResposta(mensagens, systemPrompt = "Você é a IA do CoreBot.", imageUrl = null) {
        if (this.mode === 'MOCK') {
        ...
        try {
            ...
            if (imageUrl) {
                services_1.logger.info("👁️ Imagem detectada! Alternando para modelo Vision.");
            ...
            services_1.logger.error('Erro na API IA');
            if (error.status === 429)
                return "⏳ Rate limit atingido. Tente daqui a pouco.";
            if (error.status === 402)
                return "💸 Cota de IA esgotada!";
            if (error.status === 400 && imageUrl)
...
async function isAuthorized(member) {
    ...
    try {
        const config = await (0, services_1.getGuildConfig)(member.guild.id);
        ...
        services_1.logger.warn(`Error checking auth for guild ${member.guild.id}`, { error });
    }
    return false;
}
async function parseAdminCommand(userMessage) {
    try {
        const response = await llmService.gerarResposta([{ role: 'user', content: userMessage }], ADMIN_PROMPT);
        ...
        services_1.logger.error('Erro ao parsear comando admin');
        return { action: 'none', params: {} };
    }
}
function resolveChannel(guild, channelRef) {
    ...
    if (/^\d+$/.test(channelRef)) {
        return guild.channels.cache.get(channelRef) || null;
    }
    const cleanName = channelRef.toLowerCase().replace('#', '');
    return guild.channels.cache.find(c => c.name.toLowerCase() === cleanName) || null;
}
...
async function executeAdminAction(message, action, params) {
    ...
    if (dbConnected && action !== 'none' && action !== 'server_info' && action !== 'user_info') {
        try {
            await (0, services_1.logAudit)(guild.id, message.author.id, `AI_ADMIN_${action.toUpperCase()}`, undefined, params);
        }
        catch (error) {
            services_1.logger.warn('Erro ao logar ação admin no DB');
        }
    }
    try {
        switch (action) {
            ...
            case 'kick': {
                ...
                if (!member)
                    return '❌ Usuário não encontrado.';
                try {
                    await member.send(`⚠️ **Expulso do CoreBot!**\nMotivo: ${params.reason || 'Não especificado'}`);
                }
                catch { }
            ...
            case 'ban': {
                ...
                if (!member)
                    return '❌ Usuário não encontrado.';
                try {
                    await member.send(`🔨 **Banido do CoreBot!**\nMotivo: ${params.reason || 'Não especificado'}`);
                }
                catch { }
            ...
            case 'unban': {
                if (!params.targetUserId)
                    return '❌ Preciso do ID do usuário.';
            ...
            case 'warn': {
                const member = message.mentions.members?.first();
                if (!member)
                    return '❌ Mencione o usuário.';
                try {
                    await member.send(`⚠️ **Aviso no CoreBot!**\nMotivo: ${params.reason || 'Não especificado'}`);
                }
                catch { }
            ...
            case 'create_channel': {
                const channelType = params.type === 'voice' ? discord_js_1.ChannelType.GuildVoice : discord_js_1.ChannelType.GuildText;
                const options = {
                ...
                if (params.staffOnly && STAFF_ROLE_ID) {
                    options.permissionOverwrites = [
                        { id: guild.id, deny: [discord_js_1.PermissionFlagsBits.SendMessages] },
                        { id: STAFF_ROLE_ID, allow: [discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.ViewChannel] }
                    ];
                }
            ...
            case 'slowmode': {
                const channel = (params.channelName ? resolveChannel(guild, params.channelName) : message.channel);
                if (!channel)
                    return '❌ Canal não encontrado.';
            ...
            case 'give_role': {
                ...
                if (!role)
                    return `❌ Cargo "${params.roleName}" não encontrado.`;
            ...
            case 'send_message': {
                ...
                if (!channel)
                    return '❌ Canal não encontrado.';
                const content = await llmService.gerarResposta([{ role: 'user', content: `Gere uma mensagem: ${params.prompt || 'Se apresente'}` }], 'Você é a CoreBotIA. Responda de forma amigável. Pode usar emojis.');
            ...
            case 'reminder': {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        await message.channel.send(`⏰ ${message.author}, **Lembrete:** ${params.message || 'Lembrete!'}`);
                    }
                }, duration);
            ...
            case 'partnership_add': {
                ...
                await (0, partnerships_service_1.createPartnership)(guild.id, params.partnerGuildId, params.partnerGuildName, message.author.id, { invite: params.invite, description: params.description });
                return `🤝 Parceria com **${params.partnerGuildName}** adicionada!`;
            }
            case 'partnership_list': {
                const partnerships = await (0, partnerships_service_1.listPartnerships)(guild.id);
                if (partnerships.length === 0)
                    return '📋 Nenhuma parceria ativa.';
                const list = partnerships.map((p, i) => `${i + 1}. **${p.partner_guild_name}**${p.partner_invite ? ` — ${p.partner_invite}` : ''}`).join('\n');
                return `🤝 **Parcerias Ativas (${partnerships.length}):**\n${list}`;
            }
            case 'partnership_remove': {
                if (!params.partnerGuildId)
                    return '❌ Preciso do ID do servidor parceiro.';
                const removed = await (0, partnerships_service_1.removePartnership)(guild.id, params.partnerGuildId);
                return removed ? '✅ Parceria removida.' : '❌ Parceria não encontrada.';
            }
            case 'create_custom_command': {
                if (!params.commandName || !params.actions)
                    return '❌ Preciso do commandName e array de actions.';
                // Passa as ações recebidas pro Core de Custom Commands
                try {
                    await service_1.customCommandService.create(guild.id, params.commandName, params.description || 'Comando criado pela CoreIA', null, Array.isArray(params.actions) ? params.actions : [params.actions], message.author.id, []);
                    return `✨ Comando **/${params.commandName}** criado dinamicamente com sucesso e já está disponível no servidor!`;
                }
                catch (error) {
                    services_1.logger.error('Erro no create_custom_command via IA', error);
        ...
        services_1.logger.error('Erro ao executar ação admin');
...
async function runChatGeralLogic(message) {
    ...
    if (message.attachments.size > 0) {
        ...
        if (attachment?.contentType?.startsWith('image/')) {
            ...
            if (conversasAtivas.has(message.author.id))
                deveResponder = true;
        }
    }
    ...
    if (deveResponder) {
        try {
            // 🛑 Check AI Limit
            const limitCheck = await (0, services_1.checkAiLimit)(message.author.id, guildId, message.guild.ownerId);
            if (!limitCheck.allowed) {
                const limitMsg = (0, services_1.getLimitMessage)(limitCheck.plan, limitCheck.limit);
                const embed = new discord_js_1.EmbedBuilder()
            ...
            // ✅ Increment Usage
            await (0, services_1.incrementAiUsage)(message.author.id, guildId);
            // Analytics: rastrear resposta IA
            (0, analytics_service_1.trackEvent)(guildId, 'ai_response').catch(() => { });
            // Branding: Free/Starter = com marca d'água
            const brandingFooter = await (0, branding_service_1.getBrandingFooter)(message.guild.ownerId);
            const respostaFinal = (0, branding_service_1.applyBranding)(resposta, brandingFooter);
...
async function runChatPrivadoLogic(message) {
    // 🛑 Check AI Limit
    const limitCheck = await (0, services_1.checkAiLimit)(message.author.id, message.guild.id, message.guild.ownerId);
    if (!limitCheck.allowed) {
        const limitMsg = (0, services_1.getLimitMessage)(limitCheck.plan, limitCheck.limit);
        const embed = new discord_js_1.EmbedBuilder()
            .setTitle(limitMsg.title)
    ...
    try {
        ...
        // ✅ Increment Usage
        await (0, services_1.incrementAiUsage)(message.author.id, message.guild.id);
        ...
        services_1.logger.error('Erro no chat privado');
...
client.on('messageCreate', async (message) => {
    ...
    if (message.guild) {
        (0, analytics_service_1.trackEvent)(message.guild.id, 'message').catch(() => { });
    }
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (!customId.startsWith('gif_retribuir:') && !customId.startsWith('gif_action:'))
        return;
    try {
        ...
        const { gifService } = await Promise.resolve().then(() => __importStar(require('../../shared/services/gif.service')));
        ...
        services_1.logger.error('Erro ao processar botão GIF:', { error });
        try {
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.commandName === 'construir-servidor') {
        ...
        try {
            ...
            const limitCheck = await (0, services_1.checkServerGenLimit)(interaction.user.id);
            if (!limitCheck.allowed) {
                const planNames = { free: 'Free', starter: 'Starter', pro: 'Pro', ultimate: 'Ultimate' };
                const limitEmbed = new discord_js_1.EmbedBuilder()
            ...
            // 1. Gerar Arquitetura
            const schema = await server_builder_service_1.serverBuilder.generateServerPlan(theme);
            if (!schema) {
            ...
            // 2. Construir
            await server_builder_service_1.serverBuilder.buildServer(guild, schema, async (msg) => {
                services_1.logger.info(msg);
            });
            // ✅ Increment Usage
            await (0, services_1.incrementServerGenUsage)(interaction.user.id);
        ...
        try {
            ...
            const schema = await server_builder_service_1.serverBuilder.generateAdjustmentPlan(acao);
            ...
            // 2. Aplicar Ajustes
            await server_builder_service_1.serverBuilder.applyAdjustments(guild, schema, async (msg) => {
                services_1.logger.info(msg);
            });
            await interaction.followUp({
    ...
    // ═══════════════════════════════════════════════════════════════════════════
        ...
        if (MODULE_COMMANDS.includes(interaction.commandName))
            return;
        try {
            const guildId = interaction.guildId;
            if (guildId) {
                ...
                services_1.logger.info(`🔍 Buscando comando personalizado: ${interaction.commandName} para guild ${guildId}`);
                const customCommand = await service_1.customCommandService.get(guildId, interaction.commandName);
                if (customCommand && customCommand.enabled) {
                    services_1.logger.info(`✅ Comando encontrado: ${customCommand.name}. Executando...`);
                    await engine_1.commandEngine.execute(interaction, customCommand.actions);
                }
                else {
                    services_1.logger.warn(`⚠️ Comando personalizado não encontrado ou desativado: ${interaction.commandName}`);
            ...
            services_1.logger.error(`❌ Erro crítico ao processar comando personalizado ${interaction.commandName}:`, { error });
            if (interaction.deferred) {
...
const commands = [
    ...
    new discord_js_1.SlashCommandBuilder()
        ...
        .setRequired(true)),
    ...events_4.PRIVATE_CALLS_EVENTS.commands,
    ...events_3.TICKET_EVENTS.commands
];
...
async function registerCommands(clientId) {
    ...
    try {
        ...
        services_1.logger.info(`Registrando ${commands.length} slash commands: [${commandNames}]...`);
        await rest.put(discord_js_3.Routes.applicationCommands(clientId), { body: commands });
        services_1.logger.info('✅ Slash commands registrados com sucesso no Discord!');
        services_1.logger.info('⚠️ Nota: Comandos globais podem levar até 1 hora para atualizar em todos os servidores. Se precisar urgente, reinicie o cliente Discord (Ctrl+R).');
    }
    catch (error) {
...
client.once('clientReady', async () => {
    services_1.logger.info(`🤖 COREIA CORE BOT (Multifuncional) - ONLINE`);
    ...
    try {
        ...
        if (connected) {
            ...
            try {
                const dbGuildsResult = await (0, services_1.query)('SELECT guild_id FROM guild_configs');
                ...
                if (missingGuilds.size > 0) {
                    services_1.logger.info(`🔄 Sincronizando ${missingGuilds.size} servidores ausentes no banco...`);
                    for (const guild of missingGuilds.values()) {
                        await (0, services_1.upsertGuildConfig)(guild.id, { ia_enabled: true });
                        services_1.logger.info(`✅ [Sync] Config criada para ${guild.name}`);
                    }
                }
                else {
                    services_1.logger.info('✅ Todos os servidores estão sincronizados no banco.');
                }
            }
            catch (error) {
                services_1.logger.error('❌ Erro na sincronização de guilds:', { error });
            }
        }
    }
    catch (error) {
...
// 🌌 MÓDULOS INTEGRADOS
// ═══════════════════════════════════════════════════════════════════════════
(0, events_1.setupWelcomeEvents)(client);
(0, events_2.setupAutoModEvents)(client);
(0, events_3.setupTicketEvents)(client);
(0, events_4.setupPrivateCallsEvents)(client);
// ═══════════════════════════════════════════════════════════════════════════
// 🆕 GUILD EVENTS
// ═══════════════════════════════════════════════════════════════════════════
client.on('guildCreate', async (guild) => {
    services_1.logger.info(`📥 Entrei em um novo servidor: ${guild.name} (${guild.id})`);
    try {
        // Inicializar configuração no banco de dados
        await (0, services_1.upsertGuildConfig)(guild.id, { ia_enabled: true });
        services_1.logger.info(`✅ Configuração inicial criada para ${guild.name}`);
        // Log analytics
        (0, analytics_service_1.trackEvent)(guild.id, 'guild_join').catch(() => { });
    }
    catch (error) {
        services_1.logger.error(`❌ Erro ao criar config para ${guild.name}:`, { error });
    }
});
client.on('guildDelete', async (guild) => {
    services_1.logger.info(`📤 Removido do servidor: ${guild.name} (${guild.id})`);
    try {
        // Opcional: Limpar dados ou marcar como inativo
        // Por enquanto mantemos a config para caso o bot volte
        (0, analytics_service_1.trackEvent)(guild.id, 'guild_leave').catch(() => { });
    }
    catch (error) {
        services_1.logger.error(`❌ Erro ao processar saída de ${guild.name}:`, { error });
    }
});
client.login(TOKEN).catch(error => {
    services_1.logger.error('Falha no login:', error);
    process.exit(1);
});
process.on('SIGINT', async () => {
    services_1.logger.info('Encerrando CoreIA...');
