...
const TOKEN = services_1.config.get('DISCORD_TOKEN_VIP');
const ID_CARGO_STAFF = services_1.config.get('VIP_STAFF_ROLE_ID');
const ID_CARGO_VIP = services_1.config.get('VIP_CARGO_VIP_ID');
const ID_CATEGORIA_TICKETS = services_1.config.get('VIP_CATEGORIA_TICKETS_ID');
const ID_CANAL_COMPRAS = services_1.config.getOptional('VIP_CANAL_COMPRAS_ID');
const ID_CANAL_LOGS = services_1.config.getOptional('VIP_CANAL_LOGS_ID');
if (!services_1.config.isValidDiscordToken(TOKEN)) {
...
const CHAVE_PIX = services_1.config.getOptional('VIP_CHAVE_PIX') || 'd065a8bb-e382-45b9-91bd-d25a6c4aa8f9';
const VALOR_VIP = services_1.config.getOptional('VIP_VALOR') || 'R$ 10,00';
// ═══════════════════════════════════════════════════════════════════════════
// 🤖 CLIENT
// ═══════════════════════════════════════════════════════════════════════════
...
// 🔧 FUNÇÕES HELPER
// ═══════════════════════════════════════════════════════════════════════════
function criarEmbedCoreIA(options = {}) {
...
async function enviarLog(guild, embed) {
    ...
    try {
        ...
        services_1.logger.error('Erro ao enviar log');
    }
}
async function gerarTranscript(channel, limite = 100) {
    try {
        ...
        services_1.logger.error('Erro ao gerar transcript');
...
// ═══════════════════════════════════════════════════════════════════════════
client.once('ready', () => {
    services_1.logger.info(`${EMOJIS.FOGUETE} Bot ${client.user?.tag} está online!`);
    const commands = [
        {
            name: 'painel-ticket',
    ...
    services_1.logger.info(`${EMOJIS.SUCESSO} Comando /painel-ticket registrado!`);
});
...
async function handleButton(interaction) {
    ...
    if (customId === 'abrir_ticket') {
        ...
        const ticketExistente = guild.channels.cache.find(channel => channel.name === ticketName);
        if (ticketExistente) {
            const embed = criarEmbedCoreIA({
                cor: CORES.AVISO,
        ...
        try {
            const channel = await guild.channels.create({
                ...
                permissionOverwrites: [
                    ...
                    { id: user.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.AttachFiles, discord_js_1.PermissionFlagsBits.ReadMessageHistory] },
                    { id: ID_CARGO_STAFF, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.ManageMessages, discord_js_1.PermissionFlagsBits.ReadMessageHistory] },
                ],
            });
            ...
            if (dbConnected) {
                try {
                    await (0, services_1.createTicket)(guild.id, channel.id, user.id, 'VIP');
                    await (0, services_1.logAudit)(guild.id, user.id, 'VIP_TICKET_CREATED', channel.id, {
                        channel_name: channel.name
                    });
                }
                catch (error) {
                    services_1.logger.warn('Erro ao registrar ticket VIP no DB');
                }
            }
            ...
            const rowUsuario = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
                .setCustomId('copiar_pix')
                .setLabel('Copiar PIX')
                .setStyle(discord_js_1.ButtonStyle.Primary)
                .setEmoji(EMOJIS.COPIAR), new discord_js_1.ButtonBuilder()
                .setCustomId('preciso_ajuda')
                .setLabel('Preciso de Ajuda')
                .setStyle(discord_js_1.ButtonStyle.Secondary)
                .setEmoji(EMOJIS.AJUDA));
            const rowStaff = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
                .setCustomId('confirmar_pagamento')
                .setLabel('Confirmar Pagamento')
                .setStyle(discord_js_1.ButtonStyle.Success)
                .setEmoji(EMOJIS.SUCESSO), new discord_js_1.ButtonBuilder()
                .setCustomId('cancelar_ticket')
                .setLabel('Cancelar')
                .setStyle(discord_js_1.ButtonStyle.Danger)
                .setEmoji(EMOJIS.LIXO), new discord_js_1.ButtonBuilder()
                .setCustomId('adicionar_membro')
                .setLabel('Adicionar')
                .setStyle(discord_js_1.ButtonStyle.Secondary)
                .setEmoji(EMOJIS.ADICIONAR), new discord_js_1.ButtonBuilder()
    ...
    // ✅ CONFIRMAR PAGAMENTO
        ...
        try {
            ...
            if (dbConnected) {
                ...
                await (0, services_1.logAudit)(guild.id, interaction.user.id, 'VIP_TICKET_CLOSED', channel.id, {
            ...
            services_1.logger.error('Erro ao confirmar pagamento');
            const embed = criarEmbedCoreIA({
                cor: CORES.ERRO,
                titulo: `${EMOJIS.ERRO} Erro`,
...
async function handleUserSelect(interaction) {
    ...
    if (customId === 'select_adicionar_membro') {
        try {
            ...
            services_1.logger.error('Erro ao adicionar membro');
        ...
        try {
            ...
            services_1.logger.error('Erro ao remover membro');
...
// 📝 HANDLER DE MODAIS
// ═══════════════════════════════════════════════════════════════════════════
async function handleModal(_interaction) {
...
client.once('ready', async () => {
    services_1.logger.info(`💎 Bot TicketVIP ${client.user?.tag} está online!`);
    try {
...
process.on('SIGINT', async () => {
    services_1.logger.info('Desligando Bot Ticket VIP...');
