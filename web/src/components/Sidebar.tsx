...
interface SidebarProps {
    ...
    serverIcon?: string; // Direct URL for server icon
    items: MenuItem[];
    backLink?: {
        href: string;
        label: string;
    };
    user?: any; // Session user if needed
}

export function Sidebar({ title, logo, serverIcon, items, backLink }: SidebarProps) {
    const t = useTranslations("dashboard.sidebar");
    const pathname = usePathname();

    return (
        <aside className="w-72 border-r border-white/5 bg-[#060609] flex flex-col h-screen shrink-0">
