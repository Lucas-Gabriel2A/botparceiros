...
function loadCategories(): TicketCategory[] {
    try {
...
function saveCategories(categories: TicketCategory[]): boolean {
    try {
        fs.writeFileSync(CATEGORIES_FILE, JSON.stringify(categories, null, 2), 'utf8');
        return true;
    } catch (error) {
        logger.error('Erro ao salvar categorias');
        return false;
    }
}
...
async function createTranscript(channel: TextChannel, closedBy: GuildMember): Promise<string | null> {
    try {
        ...
        logger.error('Erro ao criar transcript');
        return null;
    }
}

async function sendTranscriptToLogs(guild: Guild, transcript: string, ticketInfo: TicketInfo): Promise<void> {
    if (!LOGS_TICKETS) return;

    try {
        const logsChannel = await guild.channels.fetch(LOGS_TICKETS) as TextChannel | null;
...
${THEME.emojis.planet} **Categoria:** ${ticketInfo.category}
        ...
        logger.error('Erro ao enviar transcript para logs');
    }
}
...
async function createTicket(interaction: StringSelectMenuInteraction, categoryId: string): Promise<void> {
    const categories = loadCategories();
...
${THEME.emojis.rocket} Nossa equipe responderá em breve!
        ...
        if (dbConnected) {
            try {
                ...
                logger.warn('Erro ao registrar ticket no DB');
            }
        }
...
async function closeTicket(interaction: ButtonInteraction, userId: string, categoryName: string): Promise<void> {
    ...
    setTimeout(async () => {
        try {
            ...
            if (dbConnected) {
                ...
                await logAudit(interaction.guild!.id, member.id, 'TICKET_CLOSED', channel.id, {
                    category: categoryName,
...
client.once('ready', async () => {
    logger.info(`🌌 NEXSTAR TICKETS BOT - ONLINE`);
    ...
    if (!fs.existsSync(CATEGORIES_FILE)) {
        saveCategories([]);
        logger.info('📁 Arquivo de categorias criado.');
    }
});
    ...
    try {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (commandName === 'ticket-painel') {
                ...
                await sendTicketPanel(interaction.channel as TextChannel);
                await interaction.reply({
...
client.once('ready', async () => {
    logger.info(`✨ Bot Tickets ${client.user?.tag} está online!`);
...
process.on('SIGINT', async () => {
    logger.info('Desligando Nexstar Tickets...');
    await closePool();
    client.destroy();
    process.exit(0);
});
